...
X_Locations = vector("list", N_Patches)                                           # Location of prey
Y_Locations = vector("list", N_Patches)                                           # 

set.seed(seed)                                                                    # Reset Seed

N_PerPatch = rpois(N_Patches, 200)                                                # Create some prey
for(i in 1:N_Patches){
 X_Locations[[i]] = rnorm(N_PerPatch[i], runif(1,-2500,2500), rpois(1, 350))+100
 Y_Locations[[i]] = rnorm(N_PerPatch[i], runif(1,-2500,2500), rpois(1, 350))+100
}

food_locs = data.frame(X=c(unlist(X_Locations)), Y=c(unlist(Y_Locations)))

X_Locations_per_step = vector("list",steps)                                       # Location of prey
Y_Locations_per_step = vector("list",steps)                                       #

for(i in 1:steps){
 X_Locations_per_step[[i]] = X_Locations
 Y_Locations_per_step[[i]] = Y_Locations
}                                                                 
...
for(i in 1:N_Patches){
X_Locations[[i]] = rnorm(N_PerPatch[i], runif(1,-2500, 2500), rpois(1, 350)) + 100  # Make some patches of prey
Y_Locations[[i]] = rnorm(N_PerPatch[i], runif(1,-2500, 2500), rpois(1, 350)) + 100  #
}
...
plot(loc.x, loc.y, typ="l", ylim = c(-3500, 3500), xlim = c(-3500, 3500))        # Plot prey items
 for(i in 1:N_Patches){                                                          #
  points(X_Locations[[i]], Y_Locations[[i]], col="red", pch=".")                 #
   }                                                                             #
...
for(s in (Lags+1): (steps-1)){                                                  
 X_Locations = X_Locations_per_step[[s]]
 Y_Locations = Y_Locations_per_step[[s]]
...
############################################################################### Now check for an encounter 
 ...
 for(i in 1:N_Patches){                                                             # For each patch
  Scrap = rep(NA,length(X_Locations[[i]]))

  for(j in 1:length(X_Locations[[i]])){                                             # For each mushroom in patch
    Scrap[j] =  dist(loc.x[s],X_Locations[[i]][j],loc.y[s],Y_Locations[[i]][j]);    # Calculate the distance from the forager to the mushroom
    if(Scrap[j]<Thresh){                                                            # If the forager is closer than the visual radius slash encounter threshold
     points(X_Locations[[i]][j],Y_Locations[[i]][j], col="blue",pch=20)             # Plot a hit

       for(allgone in s:(steps-1)){
       X_Locations_per_step[[allgone]][[i]][j] = 99999                              # If this run is for destructive foraging
       X_Locations_per_step[[allgone]][[i]][j] = 99999                              # disappear food for all future timesteps
      }
 }}
